...
AssociationRulesProducer, CARuleMiner, TechnicalInformationHandler {
   ...
   <!-- options-end -->
	...
	private void findLargeItemSets() throws Exception {
		...
		try {
			FileReader in =
					new FileReader(serFileName+".log");
		...
		int rules_i=0, prevNumRules = 0;
		boolean recoverRules = false;
		try {
			FileReader in =
					new FileReader(serFileName+"Rules.log");
			BufferedReader br_in = new BufferedReader(in);
			String line = "", prev = "", end = "End";
			while((line = br_in.readLine()) != null) {
				prev = line;
				if(prev != "" ) {
					String words[] = prev.split(" ");
					if(words[0].compareTo(end) == 0) {
						prevNumRules += Integer.parseInt(words[2]);
					}
				}
			}
			if(prev != "") {
				String words[] = prev.split(" ");
				rules_i = Integer.parseInt(words[1]);
				if(words[0].compareTo(end) != 0) {
					rules_i--;
					recoverRules = true;
				}
		...
		if(recoverRules){
			findRulesQuickly();
			try {
				FileWriter out = new FileWriter(serFileName+"Rules.log",true);
				out.write("Start "+i+"\n");
				out.close();
			}catch(IOException ioex) {
			...
			try {
				FileReader in =
						new FileReader(serFileName+".out");
				BufferedReader br_in = new BufferedReader(in);
				String line = "";
				StringBuffer rulesText = new StringBuffer();
				int countRules = 0;
				while((line = br_in.readLine()) != null && countRules < prevNumRules) {
				...
				ObjectInputStream in = new ObjectInputStream(fileIn);
				FastVector[][] recRules = new FastVector[rules_i][6];
				for(int for_i=0;for_i<rules_i;for_i++){
					for(int for_j=0;for_j<6;for_j++)
						recRules[for_i][for_j] = new FastVector();
					recRules[for_i] = (FastVector[]) in.readObject();;
				}*/
				/*FileOutputStream fileOut = new FileOutputStream(serFileName+".out",true);
				ObjectOutputStream out = new AppendingObjectOutputStream(fileOut);
				for(int for_i=0;for_i<rules_i;for_i++){
					out.writeObject(recRules[for_i]);
				}
				out.writeObject(m_allTheRules);*/
				FileWriter out = new FileWriter(serFileName+".out",true);
				out.write(rulesText.toString());
				out.write(rulesToString(prevNumRules));
				out.close();
				//fileOut.close();
			}
			...
			try {
				FileWriter out = new FileWriter(serFileName+"Rules.log",true);
				out.write("End "+i+" "+m_allTheRules[0].size()+"\n");
				out.close();
			}catch(IOException ioex) {
				ioex.printStackTrace();
			}
		...
		do {
			...
			try {
				FileOutputStream fileOut = new FileOutputStream(serFileName+(i % 2)+".ser");
			...
			try {
				FileWriter out =
						new FileWriter(serFileName+".log",true);
				out.write("End "+i+"\n");
				out.close();
			}catch(IOException ioex) {
				ioex.printStackTrace();
			...
			try {
				FileWriter out =
						new FileWriter(serFileName+"Rules.log",true);
				out.write("Start "+i+"\n");
				out.close();
			}catch(IOException ioex) {
				ioex.printStackTrace();
			...
			try {
				FileWriter fileOut = 
							new FileWriter(serFileName+".out",true);
				//out.writeObject(m_allTheRules);
				
				fileOut.write(rulesToString(prevNumRules));
				fileOut.close();
			}
			catch(IOException ioexp) {
				;
	...
	public String rulesToString(int prevNumRules) {
		...
		StringBuffer text = new StringBuffer();
		int i;
		for ( i = 0; i < m_allTheRules[0].size(); i++) {
			...
			String n = Utils.doubleToString((double) prevNumRules + i + 1,
					(int) (Math.log(m_numRules) / Math.log(10) + 1), 0);
